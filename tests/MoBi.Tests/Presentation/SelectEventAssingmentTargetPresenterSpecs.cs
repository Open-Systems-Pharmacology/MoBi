...
namespace MoBi.Presentation
{
   ...
   internal class When_drilling_into_an_individual_building_block_with_entities_set_via_path : concern_for_SelectEventAssignmentTargetPresenter
   {
      private IndividualBuildingBlock _individualBB;
      private IReadOnlyList<ObjectBaseDTO> _firstLevelChildren;
      private IReadOnlyList<ObjectBaseDTO> _secondLevelChildren;
      ...
      protected override void Context()
      {
         base.Context();
         _individualBB = new IndividualBuildingBlock().WithName("IBB");
         _individualBB.Add(new IndividualParameter { Path = new ObjectPath("Organism", "Liver", "Volume") });
         _individualBB.Add(new IndividualParameter { Path = new ObjectPath("Organism", "Kidney", "Volume") });
         A.CallTo(() => _objectBaseDTOMapper.MapFrom(A<IObjectBase>._)).ReturnsLazily(x => new ObjectBaseDTO(x.Arguments.Get<IObjectBase>(0)));
      }
      ...
      [Observation]
      public void should_expand_to_organism_and_then_liver_kidney()
      {
         _firstLevelChildren.Select(x => x.ObjectBase.Name).ShouldOnlyContain("Organism");
         _secondLevelChildren.Select(x => x.ObjectBase.Name).ShouldOnlyContain("Liver", "Kidney");
      }
   }

   internal class When_getting_children_of_an_individual_building_block_twice : concern_for_SelectEventAssignmentTargetPresenter
   {
      private IndividualBuildingBlock _individualBB;
      private IReadOnlyList<ObjectBaseDTO> _firstExpansionChildren;
      private IReadOnlyList<ObjectBaseDTO> _organismChildren;
      ...
      {
         var individualBBDTO = _objectBaseDTOMapper.MapFrom(_individualBB);
         _firstExpansionChildren = sut.GetChildren(individualBBDTO);
         // simulate the tree calling GetChildren a second time on the same IBB (e.g. refresh, re-expand)
         sut.GetChildren(individualBBDTO);
         // ...now try to drill into the Organism returned from the FIRST expansion
         _organismChildren = sut.GetChildren(_firstExpansionChildren[0]);
      }

      [Observation]
      public void a_sub_container_from_the_first_expansion_should_still_resolve_its_children()
      {
         _organismChildren.Select(x => x.ObjectBase.Name).ShouldOnlyContain("Liver");
      }
   }
}