...
{
   public interface IParameterStartValueDimensionRetriever
   {
   ...
   internal class ParameterStartValueDimensionRetriever : IParameterStartValueDimensionRetriever
   {
      ...
      {
         var parameter = psv.Path.TryResolve<IParameter>(simulation.Model.Root);
      ...
      private static IParameter searchParameterInContainers(IObjectPath parameterPath, IMoBiSpatialStructure spatialStructure)
      {
         ...
         {
            parameter = parameterPath.TryResolve<IParameter>(top);
            if (parameter != null) break;
         }
         if (parameter == null)
         {
            parameter = parameterPath.TryResolve<IParameter>(spatialStructure.NeighborhoodsContainer);
         }
         return parameter;
      }
