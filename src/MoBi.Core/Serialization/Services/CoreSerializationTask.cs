...
{
   public interface ICoreSerializationTask : ISerializationTask
   {
      ...
      /// <summary>
      ///    Loads all instances of the element type <typeparamref name="T" /> from the <paramref name="fileName" />.
   ...
   public class CoreSerializationTask : ICoreSerializationTask
   {
      ...
      public CoreSerializationTask(IXmlSerializationService xmlSerializationService, 
         IContextPersistor contextPersistor,
         IObjectTypeResolver objectTypeResolver, 
         IDialogCreator dialogCreator,
         IProjectConverterLogger projectConverterLogger,
         IMoBiContext context, 
         IPostSerializationStepsMaker postSerializationSteps, 
      ...
      public virtual T Load<T>(string fileName, bool resetIds = false) => LoadAll<T>(fileName, resetIds).FirstOrDefault();

      public IReadOnlyList<T> LoadAll<T>(string fileName, bool resetIds = false) => loadMany<T>(fileName, x => x, resetIds).ToList();

      protected IEnumerable<T> loadMany<T>(string fileName, Func<IReadOnlyList<XElement>, IReadOnlyList<XElement>> elementSelector, bool resetIds = false)
      {
