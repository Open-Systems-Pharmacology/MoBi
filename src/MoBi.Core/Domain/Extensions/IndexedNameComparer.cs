...
namespace MoBi.Core.Domain.Extensions
{
   public class IndexedNameComparer : IComparer<string>
   {
      public int Compare(string x, string y)
      {
         ...
         // both have a numeric suffix and neither is zero-padded, compare them as numbers
         var xIsZeroPadded = suffixX.Length > 1 && suffixX[0] == '0';
         var yIsZeroPadded = suffixY.Length > 1 && suffixY[0] == '0';
         if (int.TryParse(suffixX, out var numX) && int.TryParse(suffixY, out var numY)
             && !xIsZeroPadded && !yIsZeroPadded)
            return numX.CompareTo(numY);

         // fall back to string comparison if either suffix is not numeric or is zero-padded
